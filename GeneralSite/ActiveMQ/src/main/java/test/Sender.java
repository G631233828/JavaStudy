...
public class Sender {
	...
	public  static void execute(int i)  throws JMSException, InterruptedException{
		// 第一步，建立ConnectionFactory工厂对象，需要填入用户名，密码，以及要连接的地址，均使用默认的即可，默认端口：“tcp://localhost:616161”
				...
				Destination destination = session.createQueue("MQ_notification");
				...
				s.setDateTime(format.format(new Date()));
		        s.setName("fliay");
		        s.setRemark("到校通知"+i);
		        ...
		        producer.send(destination, msg);//i是优先级，20000存活时间
