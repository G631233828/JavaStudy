...
/**
 * @author fliay
 * 一、Lamdba 表达式的基础语法：java8中引入了一个新的操作符“->” 称为箭头操作符或Lamdba操作符
 * 		箭头操作符将lamdba表达式拆分为：
 *    左侧：Lamdba 表达式的参数列表
 *    右侧：Lamdba 表达式中所需要执行的功能，即Lamdba体
 *    语法格式1：无参数，无返回值
...
public class TestLamdba {
	...
	/**
	 * jdk1.8之前的版本与jdk1.8使用Runnable的区别
	 */
	@Test
	public void test1(){
		Runnable r1 = new Runnable(){
			public void run() {
			System.out.println("jdk1.8以前启动线程的方法");
			}
		};
		r1.run();
		
		System.out.println("------------版本分割线-----JDK1.8-------------------");
		//使用Lamdba表达式的方法使用线程
		Runnable r2 = ()->System.out.println("使用Lamdba表达式的方法");
		r2.run();
	}
	/**
	 * 模拟无参数，无返回值的Lamdba表达式
	 */
	@Test
	public void test2(){
		Runnable r = ()-> System.out.println("你好！Lamdba,无参，无返回值");
		r.run();
	}
